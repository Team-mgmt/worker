...
# /usr/local/bin/shelfalign-otel-refresh and driven both from after_install.sh (once,
# at deploy time) and from a systemd timer (every 5 min at runtime).
#
# Runtime config is read from /etc/shelfalign-otel/refresh.conf so the script is fully
# self-contained and does not depend on the (wiped-on-deploy) app directory.
set -euo pipefail

CONF_FILE="/etc/shelfalign-otel/refresh.conf"
COLLECTOR_CONFIG="/etc/otelcol-contrib/config.yaml"
COLLECTOR_ENV="/etc/shelfalign-otel/otel.env"
COLLECTOR_SERVICE="otelcol-contrib"

if [[ ! -f "${CONF_FILE}" ]]; then
    echo "shelfalign-otel-refresh: ${CONF_FILE} missing; nothing to do" >&2
    exit 0
fi
...
if aws ssm get-parameter --name "${OTEL_CONFIG_PARAM}" --with-decryption \
    ...
    if [[ ! -f "${COLLECTOR_CONFIG}" ]] || \
       ! cmp -s "${new_config}" "${COLLECTOR_CONFIG}"; then
        ...
        echo "shelfalign-otel-refresh: collector config updated from ${OTEL_CONFIG_PARAM}"
        changed=1
    fi
else
    echo "shelfalign-otel-refresh: failed to fetch ${OTEL_CONFIG_PARAM}; keeping existing config" >&2
fi
...
if aws ssm get-parameters-by-path --path "${SSM_PARAMETER_PATH}" --recursive \
    ...
    if [[ ! -f "${COLLECTOR_ENV}" ]] || ! cmp -s "${new_env}" "${COLLECTOR_ENV}"; then
        ...
        echo "shelfalign-otel-refresh: collector env updated ($(wc -l < "${new_env}") OTEL_* vars)"
        changed=1
    fi
else
    echo "shelfalign-otel-refresh: failed to fetch OTEL_* params from ${SSM_PARAMETER_PATH}; keeping existing env" >&2
fi
...
if [[ "${changed}" -eq 1 ]]; then
    ...
    echo "shelfalign-otel-refresh: ${COLLECTOR_SERVICE} reloaded"
else
    echo "shelfalign-otel-refresh: no changes"
fi
