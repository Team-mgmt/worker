...
MODEL_CACHE_DIR="${MODEL_CACHE_DIR:-/var/cache/shelfalign-worker/models}"
...
WHEEL_CACHE_DIR="/var/cache/shelfalign-worker/wheels"
...
configure_otel_collector() {
    ...
    mkdir -p /etc/shelfalign-otel /etc/otelcol-contrib
    ...
    umask 077
    cat > /etc/shelfalign-otel/refresh.conf <<EOF
SSM_PARAMETER_PATH=${SSM_PARAMETER_PATH}
...
EOF
    ...
    install -m 755 "${APP_DIR}/scripts/codedeploy/otel_refresh.sh" \
        /usr/local/bin/shelfalign-otel-refresh
    ...
    install -m 644 "${APP_DIR}/scripts/codedeploy/templates/otelcol-contrib-override.conf" \
        /etc/systemd/system/otelcol-contrib.service.d/shelfalign.conf
    install -m 644 "${APP_DIR}/scripts/codedeploy/templates/shelfalign-otel-refresh.service" \
        /etc/systemd/system/shelfalign-otel-refresh.service
    install -m 644 "${APP_DIR}/scripts/codedeploy/templates/shelfalign-otel-refresh.timer" \
        /etc/systemd/system/shelfalign-otel-refresh.timer
    ...
    systemctl enable --now shelfalign-otel-refresh.timer
