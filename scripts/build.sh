...
if [[ ! -d "${PROJECT_DIRECTORY}/build/qmr-web" ]]; then
  git clone git@github.com/QMR-tech/web.git "${PROJECT_DIRECTORY}/build/qmr-web"
  # We always check out to develop - this script is only used in develop builds
  git checkout develop
else
  cd "${PROJECT_DIRECTORY}/build/qmr-web" || exit 1
  git pull
fi
