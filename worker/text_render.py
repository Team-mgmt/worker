...
Mirrors qmr-web's ``TeacherExamFileService`` text-rendering logic: TEXT areas
...
# CSS font-weight → font file weight suffix (matches qmr-web's fontWeightMap).
...
class _AreaOverride(BaseModel):
    """Per-area override payload mirroring qmr-web's inner zod object.

    zod schema (qmr-web/packages/schema/src/models/exam.ts):
...
class _ExamMetadataV1(BaseModel):
    """Top-level exam.metadata payload (qmr-web's ``ExamMetadataV1Schema``).
...
def _substitute_text_variables(
    ...
    *,
    ...
    """Substitute placeholder tokens; mirrors qmr-web's ``substituteTextVariables``."""
    return (
...
def _format_exam_name(exam: Exam) -> str:
    """Mirror qmr-web ``formatExamName``: join non-empty year, title, "<n>회"."""
    parts: list[str] = []
    if exam.year:
...
async def render_text_areas_on_template(
    *,
    ...
    for area in text_areas:
        ...
        # Vertically center inside the area (mirrors qmr-web's PDFKit path).
        try:
