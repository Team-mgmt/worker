...
Use **pydantic** for all first-class schema parsing — anything that mirrors a zod schema on the qmr-web side, validates external JSON payloads (DB JSONB columns, API request bodies, S3 metadata blobs), or enforces structural invariants on untrusted input. Define a `BaseModel` subclass with explicit field types and let pydantic's `ValidationError` drive the "fall back / reject" branch; do not hand-roll `isinstance`/`if`-tree validators when a model would express the same intent.
