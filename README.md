...
ShelfAlign AI는 2026 문화체육관광부 공공데이터 활용 창업경진대회를 위해 개발된 **스마트폰 기반 도서관 서가 오배열 탐지 및 도서 탐색 서비스**입니다. 
이 레포지토리(worker)는 영상 처리, AI 객체 탐지, OCR 파이프라인, 공공데이터 수집 및 매칭을 담당하는 핵심 백엔드 서버입니다. 프론트엔드(web)로부터 전달받은 이미지를 분석하여 구조화된 결과를 반환합니다.

> ⚠️ **주의**: 본 프로젝트는 창업경진대회 출품용 MVP 버전으로 🚧 현재 일부 기능이 구현 중(WIP)에 있습니다. 모델의 학습 가중치(.pt) 및 공공 API 인증키 등은 보안상 `.gitignore` 처리되어 레포지토리에 포함되지 않습니다.
...
ShelfAlign 팀은 2026 공공데이터 활용 창업경진대회를 위해 구성되었습니다.
