...
qmr-web has a submission "name editing" feature backed by two metadata fields:
...
- Any qmr-web change. The metadata shape (`currentNameEditId`, `nameHistory`
  entry keys, `"WORKER"` source) is assumed to match qmr-web's expectations.
