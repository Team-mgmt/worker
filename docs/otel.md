...
aws ssm get-parameter --region ap-northeast-2 --name /dev/shelfalign/otel-config \
...
[Service]
Environment=OTEL_S3_BUCKET=shelfalign-otel-dev
Environment=OTEL_SERVICE_NAME=shelfalign-worker
Environment=AWS_REGION=ap-northeast-2
The config references ${env:OTEL_S3_BUCKET} and ${env:OTEL_SERVICE_NAME} — both must be exported into the collector's process env or the exporter will fail to start. Heads-up: /dev/shelfalign/otel-bucket (or similar) isn't in SSM yet — you'll need to create the bucket and either hardcode in the drop-in or add an SSM param.

4. IAM — the EC2 instance role needs s3:PutObject (and s3:PutObjectAcl if the bucket enforces ACLs) scoped to arn:aws:s3:::shelfalign-otel-dev/*. The awss3 exporter signs with the default credentials chain, which on EC2 = IMDSv2 instance role.

5. Wire the worker in shelfalign-worker.service:
...
- No bucket param in SSM — /dev/shelfalign/otel-bucket doesn't exist, so the OTEL_S3_BUCKET env has no source of truth yet. Worth adding now to match the SSM-driven pattern the rest of the worker uses.
