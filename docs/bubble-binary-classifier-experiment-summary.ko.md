...
- 실제 버블 이미지
  - S3 경로 예시:
    - `s3://dev-hi4labs-label-studio/shelfalign-worker/debug/.../__scan.png`
    - 실제 다운로드는 권한 이슈로 `dev-shelfalign-assets` 버킷 기준으로 수행
...
- 코드 번들:
  - `s3://dev-shelfalign-assets/shelfalign-worker/training/training_bundle.zip`
- 이미지 캐시 번들:
  - `s3://dev-shelfalign-assets/shelfalign-worker/training/cache_bundle.tgz`
...
aws s3 cp .\training_bundle.zip s3://dev-shelfalign-assets/shelfalign-worker/training/training_bundle.zip --profile work-sso
```

### 2. 인스턴스에서 번들 다운로드

```bash
mkdir -p ~/shelfalign-train
cd ~/shelfalign-train
aws s3 cp s3://dev-shelfalign-assets/shelfalign-worker/training/training_bundle.zip .
...
```bash
/opt/shelfalign-worker/.venv/bin/python training/src/split_problem_dataset.py \
...
```bash
/opt/shelfalign-worker/.venv/bin/python training/src/cache_problem_images.py \
  ...
  --bucket-override dev-hi4labs-label-studio=dev-shelfalign-assets
```
...
aws s3 cp cache_bundle.tgz s3://dev-shelfalign-assets/shelfalign-worker/training/cache_bundle.tgz
```

### 6. symlink 생성

```bash
cd ~/shelfalign-train/training/cache
rm -rf dev-hi4labs-label-studio
ln -s dev-shelfalign-assets dev-hi4labs-label-studio
cd ~/shelfalign-train
```
...
./ml-venv/bin/python training/src/train_tabular_ml.py \
  ...
  --s3-backup-uri s3://dev-shelfalign-assets/shelfalign-worker/training/runs/logistic_regression
```
...
- ResNet18
  - `s3://dev-shelfalign-assets/shelfalign-worker/training/runs/resnet18/`
- Logistic Regression
  - `s3://dev-shelfalign-assets/shelfalign-worker/training/runs/logistic_regression/`
