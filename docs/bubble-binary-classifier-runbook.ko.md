...
- EC2 `dev-shelfalign-worker-auto`에 Session Manager로 접속 가능하다.
- 인스턴스 내부 Python은 `/opt/shelfalign-worker/.venv/bin/python`을 사용한다.
...
```powershell
aws s3 cp .\project-2-at-2026-05-17-15-02-b24adaa3.json s3://dev-shelfalign-assets/shelfalign-worker/training/project-2-at-2026-05-17-15-02-b24adaa3.json --profile work-sso
```
...
```bash
nvidia-smi
/opt/shelfalign-worker/.venv/bin/python --version
/opt/shelfalign-worker/.venv/bin/python -c "import torch; print(torch.__version__); print(torch.cuda.is_available())"
...
```bash
aws s3 cp training/runs s3://dev-shelfalign-assets/shelfalign-worker/training/runs --recursive
```
...
/opt/shelfalign-worker/.venv/bin/python training/src/train.py \
  ...
  --batch-size 32 \
  --num-workers 2 \
  --s3-backup-uri s3://dev-shelfalign-assets/shelfalign-worker/training/runs/resnet18
```
