...
class TestLoadSsmParameters:
    ...
    async def test_loads_parameters(self):
        ...
        async def mock_paginate(**kwargs):
            yield {
                "Parameters": [
                    {"Name": "/qmr/worker/DATABASE_URL", "Value": "postgres://localhost"},
                    {"Name": "/qmr/worker/S3_BUCKET_NAME", "Value": "my-bucket"},
                ]
            }
        ...
        with (
            patch.dict(os.environ, {"SSM_PARAMETER_PATH": "/qmr/worker/"}, clear=False),
